...
LATENCYDIR = settings["HTTP_LATENCY_DIR"]
DATADIR = settings["DATA_DIR"]
COPYFILEPREFIX = settings["COPYFILE_PREFIX"]
FORMAT = settings["FORMAT"]
URL = settings["HTTP_SERVER_URL"]

def run(run_number, limit):
    opfile = open('{}http_latency_{}.txt'.format(LATENCYDIR, run_number), 'w')
    for j in range(limit):
        ...
        for i in range(2**j):
            ...
            t1 = time.time()            
            requests.post(URL, files = {"filename": open(dest, 'rb')})                
